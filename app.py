...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL', 'sqlite:///dtustakes.db'
...
def signup():
    ...
    log_event(event_type='signup', page='/signup', event_value=1,
              meta={'roll_no': user.roll_no, 'email': user.email})
...
def login():
    ...
    log_event(event_type='login', page='/login', event_value=1,
...
def dashboard():
    ...
    draw = Draw(hashed_server_seed=hashed, server_seed=server_seed, nonce=nonce)
    db.session.add(draw)
    db.session.commit()

    bets = get_user_bets(user.id)
    return render_template('dashboard.html', user=user, hashed=hashed,
                           nonce=nonce, draw=draw, bets=bets)
...
def verify():
    if 'user_id' not in session:
        return redirect(url_for('index'))
    log_event(event_type='verify_page_visit', page='/verify', event_value=1)
...
def api_bet():
    ...
    log_event(event_type='bet_placed', page='/api/bet', event_value=amount,
              meta={'draw_id': draw_id, 'bet_type': bet_type,
                    'pick': pick, 'user_id': session.get('user_id')})
...
def api_reveal():
    ...
    log_event(event_type='draw_revealed', page='/api/reveal', event_value=1,
              meta={'hashed_server_seed': hashed, 'nonce': nonce,
                    'user_id': session.get('user_id')})

    latest_paid = sum(int(b.get('won', 0) or 0)
                      for b in get_user_bets(session['user_id'])
                      if int(b.get('won', 0) or 0) > 0)
    if latest_paid > 0:
        log_event(event_type='payout', page='/api/reveal', event_value=latest_paid,
                  meta={'user_id': session.get('user_id')})
...
def api_history():
    if 'user_id' not in session:
        return jsonify({'error': 'Login required'}), 401
    return jsonify(get_user_bets(session['user_id']))
...
def api_checkwin(bet_id):
    if 'user_id' not in session:
        return jsonify({'error': 'Login required'}), 401
    return jsonify(check_win(bet_id))


@app.route('/api/verify-draw', methods=['POST'])
def api_verify_draw():
    data = request.get_json()
    if not data:
    ...
    draw_id = data.get('draw_id')
    if not draw_id:
        return jsonify({'error': 'draw_id is required'}), 400

    try:
        draw_id = int(draw_id)
    except (ValueError, TypeError):
        return jsonify({'error': 'draw_id must be a number'}), 400

    draw = Draw.query.get(draw_id)
    if not draw:
        return jsonify({'error': f'Draw #{draw_id} not found'}), 404

    if draw.open_digit is None:
        return jsonify({'error': f'Draw #{draw_id} has not been revealed yet'}), 400

    recomputed_hash = hashlib.sha256(draw.server_seed.encode()).hexdigest()
    hash_matches    = (recomputed_hash == draw.hashed_server_seed)

    return jsonify({
        'draw_id':         draw.id,
        'hashed_seed':     draw.hashed_server_seed,
        ...
        'jodi':            draw.jodi,
        'timestamp':       draw.timestamp.strftime('%Y-%m-%d %H:%M:%S'),
    })
